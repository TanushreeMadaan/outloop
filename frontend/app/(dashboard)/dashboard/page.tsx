...
const COLORS = ["#BFDBF7", "#1F7A8C", "#E1E5F2"];
const chartTick = "#5f7288";
  ...
  const stats = [
    {
      ...
      icon: (
        <svg className="h-5 w-5 text-[#022B3A]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
        </svg>
      ),
      color: "bg-[#1F7A8C] text-white",
    },
    {
  ...
  return (
    <div className="page-shell space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {stats.map((stat, i) => (
          <Card key={i} className="overflow-hidden transition-all duration-200 hover:-translate-y-0.5">
            <CardContent className="p-5">
              <div className="mb-3 flex items-center justify-between">
                <div className={`flex h-11 w-11 items-center justify-center rounded-[1rem] ${stat.color}`}>{stat.icon}</div>
              </div>
              <div>
                ...
                <div className="mt-1 flex items-baseline gap-2">
                  ...
                  <span className={`text-[10px] font-medium ${stat.label === "Pending Returns" && (summary?.overdueReturns || 0) > 0 ? "animate-pulse text-primary" : "text-muted-foreground"}`}>
                    {stat.subLabel}
                  </span>
                </div>
              </div>
            ...
            <CardContent className="h-[240px] pt-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={charts?.trends || []}>
                  <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: chartTick }} hide />
                  <Tooltip contentStyle={tooltipStyle} />
                  <Area type="monotone" dataKey="total" stroke="#1F7A8C" strokeWidth={2.5} fill="#BFDBF7" fillOpacity={0.72} />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
      ...
      <div className="mt-8">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/transactions" className="inline-flex items-center gap-1 text-sm font-medium text-primary transition-colors hover:text-[#145766]">
            View All
              ...
              <tbody className="divide-y divide-border/60">
                {summary?.recentTransactions.map((transaction: Transaction) => (
                  <tr key={transaction.id} className="table-row">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-[13px] font-medium text-primary">{transaction.department.name}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="soft-tag bg-secondary/45 text-muted-foreground">
                        {transaction.items.length} {transaction.items.length === 1 ? "Item" : "Items"}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex flex-col items-end gap-1">
                        <span className={`soft-tag ${transaction.isReturnable ? "bg-secondary text-foreground" : "bg-muted text-foreground"}`}>
                          {transaction.isReturnable ? "Returnable" : "Consumable"}
                        </span>
                        {transaction.isReturnable && transaction.expectedReturnDate && (
                          <span className="rounded-full border border-border/80 bg-secondary/30 px-2 py-0.5 text-[9px] font-medium text-muted-foreground">
